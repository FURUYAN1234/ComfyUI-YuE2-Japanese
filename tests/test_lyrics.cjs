...
const data={title:'雨の歌',lyrics:'[Verse]\n雨の道\n街の灯り\n\n[Chorus]\n明日へ',style:'Japanese pop',audio_seconds:39.5587,folder:'/home/test-reader/ComfyUI/output/audio/YuE2/test',note:'確認'};
const source=JSON.stringify(data);const shown=readableLyrics(source);
assert(!shown.includes('/home/test-reader'));assert(shown.includes('ComfyUI/output/audio/YuE2/test'));assert(shown.includes('雨の道\n街の灯り'));assert(!shown.includes('\\n'));assert(shown.includes('曲名 / Title：雨の歌'));assert.equal(JSON.parse(source).lyrics,data.lyrics);
