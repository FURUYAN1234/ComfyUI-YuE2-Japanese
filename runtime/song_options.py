...
def settings(mode='おまかせ',voice='おまかせ',genre='おまかせ',mood='おまかせ',instruments='おまかせ',bpm=0,timing='可変尺（自然な長さ）',seconds=30):
 ...
 return {'mode':mode,'voice':voice,'genre':genre,'mood':mood,'instruments':instruments,'bpm':bpm,'timing':timing,'seconds':seconds,'style':', '.join(parts)}
def prepare(brief,options,manual=None):
 o=settings(**{k:options[k] for k in ['mode','voice','genre','mood','instruments','bpm','timing','seconds']})
 style=o['style']
 if o['mode']=='手動':
